"""
wget https://huggingface.co/thewh1teagle/phonikud-onnx/resolve/main/phonikud-1.0.int8.onnx
uv run --extra gpu python examples/with_gpu.py
"""
from phonikud_onnx import Phonikud
from phonikud import lexicon
import onnxruntime as ort
import time

def main():
    print(f'Available providers: {ort.get_available_providers()}')

    sess = ort.InferenceSession("./phonikud-1.0.int8.onnx", providers=["CUDAExecutionProvider"])
    phonikud = Phonikud.from_session(session=sess)
    sentence = ["כמה אתה חושב שזה יעלה לי? אני מגיע לשם רק בערב.."] * 5
    with_diacritics = phonikud.add_diacritics(
        sentence, mark_matres_lectionis=lexicon.NIKUD_HASER_DIACRITIC
    )
    print(with_diacritics)
        

if __name__ == "__main__":
    main()
